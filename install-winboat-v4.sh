...
get_target_user(){
    ...
    # Find the active graphical desktop session (X11/Wayland).
    ...
    # Fallback if no active graphical session is found.
    ...
    if ! id "$TARGET_USER" >/dev/null 2>&1; then
        fail "Unable to detect target user."
    fi

    if [ "$TARGET_USER" = "root" ]; then
        fail "Target user cannot be root."
    fi

    ok "Target user detected: $TARGET_USER"
}
